...
タブ音声┘                                                      ↓ 会議を終了
                                        WAV 化・音量の正規化 → whisper.cpp（ローカル・無料）
                                                              ↓
...
中身は Electron ではなく **Chrome を開くだけのランチャー**（`scripts/launcher.sh`）で、
サーバーが動いていなければ起こしてから、**普段使っている Chrome** でページを開く。

### 🚨 別インスタンスの Chrome で開いてはいけない

`--app` や `--user-data-dir` で独立した Chrome を立てると、単独ウィンドウにはなるが
**画面共有ダイアログに会議のタブが出てこない**。ピッカーが列挙するのは
「同じ Chrome インスタンスのタブ」だけで、別インスタンスのタブは選べないため。
オンライン会議で相手の声を録る唯一の経路が塞がるので、この方式は採らない。

### 単独ウィンドウで使いたい場合

**普段の Chrome に PWA としてインストールする**。同じインスタンスのままアプリウィンドウになるので、
タブ音声の共有もマイクの許可もそのまま使える。

1. `http://localhost:3000` を Chrome で開く
2. アドレスバー右の **インストール** から「議事録」を追加する

インストールすると Chrome が `~/Applications/Chrome Apps.localhost/議事録.app` を作る。
`scripts/launcher.sh` はそれがあれば優先して開くので、Launchpad の「議事録」から
サーバー起動 → アプリウィンドウまで一息で通る。
...
対面はマイク1本ぶんしか音源が無いぶん、録音レベルがオンラインより 15dB ほど低くなる
（オンラインは相手の声がタブ音声というフルスケールのデジタル信号で入るため）。
**このまま whisper に渡すと、会話が明瞭に録れていても発話ごと「音楽」と判定されて捨てられる。**

そのため `pcmToWav` が WAV 化と同時に音量を揃える。全体を一律に持ち上げるのではなく、
500ms のフレームごとに見て静かな区間ほど強く持ち上げる（一律だと区間ごとの比率が
変わらないので直らない）。1 時間の音声で 2.2 秒、文字起こしの 8 分に対して誤差の範囲。

`audio.pcm` は書き換えないので、マイクが実際に拾えていたレベルは `preview.sh` で確認できる。
...
ここに出るのは**正規化する前**の生のレベルなので、対面会議で -35dB 前後でも問題ない
（文字起こしの直前に持ち上がる）。-60dB を下回っているときはマイクを疑う。
発話が1つも取れなかった会議は、エラーメッセージに実測レベルが出る。
